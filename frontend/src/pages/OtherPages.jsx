...
import { RELATIONSHIPS, QUALITY_SCORES, ER_LINKS } from '../data/db.js'
...
export function ERDiagramPage() {
  ...
  const LEGEND = [
    { color:'#c0392b', label:'customers' },
    { color:'#2980b9', label:'orders' },
    { color:'#9b59b6', label:'order_items' },
    { color:'#27ae60', label:'products' },
    { color:'#f39c12', label:'payments' },
    { color:'#e74c3c', label:'sellers' },
    { color:'#8e44ad', label:'reviews' },
    { color:'#666680', label:'geolocation' },
  ]

  return (
    <div style={{ display:'flex', flexDirection:'column', height:'calc(100vh - 64px - 64px)' }}>
      <PageHeader title="ER Diagram Viewer" sub="D3 force-directed graph · Drag nodes · Scroll to zoom · Click a table to inspect">
        ...
        <Button variant="primary" onClick={() => {
          ...
          const a = document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='olist_er.svg'; a.click()
        }}>⬇ SVG</Button>
      </PageHeader>
      ...
      <div style={{ display:'flex', gap:16, flex:1, minHeight:0 }}>
        ...
        >
          {/* Fullscreen controls overlay — shown only in fullscreen */}
          {fullscreen && (
            <div style={{
              position:'absolute', top:16, right:16, zIndex:10,
              display:'flex', gap:8,
            }}>
              <button onClick={() => svgRef.current?.__zoomOut?.()} style={{ width:36, height:36, borderRadius:8, background:'rgba(15,15,23,0.9)', border:'1px solid #1e1e2e', color:'#b0b0c8', cursor:'pointer', fontSize:16, display:'flex', alignItems:'center', justifyContent:'center' }}>－</button>
              <button onClick={() => svgRef.current?.__resetZoom?.()} style={{ width:36, height:36, borderRadius:8, background:'rgba(15,15,23,0.9)', border:'1px solid #1e1e2e', color:'#b0b0c8', cursor:'pointer', fontSize:14, display:'flex', alignItems:'center', justifyContent:'center' }}>⊙</button>
              <button onClick={() => svgRef.current?.__zoomIn?.()} style={{ width:36, height:36, borderRadius:8, background:'rgba(15,15,23,0.9)', border:'1px solid #1e1e2e', color:'#b0b0c8', cursor:'pointer', fontSize:16, display:'flex', alignItems:'center', justifyContent:'center' }}>＋</button>
              <button onClick={toggleFullscreen} style={{ height:36, padding:'0 14px', borderRadius:8, background:'rgba(192,57,43,0.8)', border:'1px solid #c0392b', color:'#fff', cursor:'pointer', fontSize:12, fontFamily:"'Space Mono',monospace" }}>⛶ Exit</button>
            </div>
          )}
          <ERDiagram svgRef={svgRef} onNodeClick={handleNodeClick} />
        </div>
        ...
        <div style={{ width:260, display:'flex', flexDirection:'column', gap:14, overflowY:'auto' }}>
              ...
              {selected ? (
                <div>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                    {ER_LINKS.filter(l => l.source===selected||l.target===selected).map((l,i) => (
                      <div key={i} style={{ background:'#16161f', border:'1px solid #1e1e2e', borderRadius:8, padding:'9px 12px' }}>
                        <div style={{ fontFamily:"'Space Mono',monospace", fontSize:10, color:'#f0828a' }}>{l.source} → {l.target}</div>
                        <div style={{ fontFamily:"'Space Mono',monospace", fontSize:9, color:'#666680', marginTop:3 }}>{l.via} · {l.card}</div>
                      </div>
                    ))}
                    {ER_LINKS.filter(l=>l.source===selected||l.target===selected).length===0 && (
          ...
          {/* All relationships */}
          <Panel>
            <PanelHeader title={`FK Relationships (${ER_LINKS.length})`} />
            <PanelBody>
              <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
                {ER_LINKS.map((r,i) => (
                  <div key={i} style={{ background:'#16161f', border:'1px solid #1e1e2e', borderRadius:8, padding:'10px 12px' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:3 }}>
                      <span style={{ fontFamily:"'Space Mono',monospace", fontSize:11, color:'#f0828a' }}>{r.source}</span>
                      <span style={{ color:'#444458' }}>→</span>
                      <span style={{ fontFamily:"'Space Mono',monospace", fontSize:11, color:'#f0828a' }}>{r.target}</span>
                    </div>
          ...
          <Panel>
            ...
            <PanelBody>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                {[['Tables','9','#27ae60'],['Relations','7','#2980b9'],['Rows','1.6M+','#f39c12'],['Columns','47','#9b59b6']].map(([l,v,c]) => (
...
export function ConnectionsPage() {
  const [log, setLog] = useState([
    ...
    {color:'#27ae60',msg:'  [ OK ] 9 tables discovered'},
  ...
  const runTest = async () => {
    ...
    const lines = [
      {color:'#3498db',msg:'[ INFO ] Connecting to db.olist.internal:5432'},
      {color:'#3498db',msg:'[ INFO ] Authenticating as analyst_user...'},
      {color:'#27ae60',msg:'  [ OK ] Authentication successful'},
      {color:'#27ae60',msg:'  [ OK ] SSL handshake complete (TLS 1.3)'},
      {color:'#3498db',msg:'[ INFO ] Scanning schema metadata...'},
      {color:'#27ae60',msg:'  [ OK ] 9 tables discovered'},
      {color:'#27ae60',msg:'  [ OK ] 47 columns mapped'},
      {color:'#27ae60',msg:'  [ OK ] 12 FK relationships detected'},
      {color:'#27ae60',msg:'  [ OK ] Round-trip latency: 12ms'},
  ...
  const DBS = [
    {emoji:'🐘',name:'PostgreSQL',desc:'Full schema introspection, FK detection, constraint mapping.',connected:true,latency:'12ms'},
    {emoji:'🐬',name:'MySQL / MariaDB',desc:'InnoDB schema extraction and stored procedure analysis.',connected:false},
    {emoji:'🟦',name:'SQL Server',desc:'MSSQL metadata extraction with T-SQL support.',connected:false},
    {emoji:'🪶',name:'SQLite',desc:'Lightweight local DB for rapid prototyping.',connected:false},
  ...
  return (
    <div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:14, marginBottom:24 }}>
        ...
        <MetricCard icon="⛁" label="Tables Discovered" value="9" delta="▲ Auto-scanned" cardColor="#2980b9" delay={50}/>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
          <Panel>
            <PanelHeader title="Active Connection — Olist DB"><Tag variant="done">● LIVE</Tag></PanelHeader>
            <PanelBody>
              {[['Engine','PostgreSQL 15.2'],['Host','db.olist.internal:5432'],['Database','olist_db'],['User','analyst_user'],['Tables','9 discovered'],['Latency','12ms avg'],['SSL','✓ Encrypted'],['Last Sync','2 minutes ago']].map(([k,v]) => (
